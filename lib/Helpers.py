...
def create_campers(empty_campers_list, sheet):
    ...
    class Camper:
        def __init__(self):
            ...
            self.next_activity = (
                ""
            )  # for recording the camper's assigned peulah for the coming session
            self.pref_1 = ""
            self.pref_2 = ""
            self.pref_3 = ""
            self.pref_4 = ""
    ...
    for i in range(sheet.ncols):
        ...
        if (
            "name" in sheet.cell_value(0, i).lower()
            or "camper" in sheet.cell_value(0, i).lower()
        ):
        ...
        # Adds bunks to each camper object -- same for loop as above
            ...
            "bunk" in sheet.cell_value(0, i).lower()
            or "tzrif" in sheet.cell_value(0, i).lower()
        ):
            for j in range(sheet.nrows - 1):
                if sheet.cell_type(j + 1, i) != xlrd.XL_CELL_EMPTY:  # Check if empty
        ...
        # Adds preferences to camper objects, up to 9 preferences -- same for loop as above
            ...
            "1" in sheet.cell_value(0, i) or "first" in sheet.cell_value(0, i).lower()
        ):  # If name becomes first + last, problem here
            for j in range(sheet.nrows - 1):
                if sheet.cell_type(j + 1, i) != xlrd.XL_CELL_EMPTY:  # Check if empty
                    empty_campers_list[j].pref_1 = sheet.cell_value(j + 1, i)
        elif (
            "2" in sheet.cell_value(0, i) or "second" in sheet.cell_value(0, i).lower()
        ):
            for j in range(sheet.nrows - 1):
                if sheet.cell_type(j + 1, i) != xlrd.XL_CELL_EMPTY:  # Check if empty
            ...
            "4" in sheet.cell_value(0, i) or "fourth" in sheet.cell_value(0, i).lower()
        ):
            for j in range(sheet.nrows - 1):
                if sheet.cell_type(j + 1, i) != xlrd.XL_CELL_EMPTY:  # Check if empty
            ...
            "7" in sheet.cell_value(0, i) or "seventh" in sheet.cell_value(0, i).lower()
        ):
            for j in range(sheet.nrows - 1):
                if sheet.cell_type(j + 1, i) != xlrd.XL_CELL_EMPTY:  # Check if empty
...
def create_activities(empty_activities_list, excel_location):
    ...
    class Activity:
        def __init__(self):
            ...
            self.capacity = (
                0
            )  # for tracking max # of campers to be placed in the activity
            self.repeatability = (
                True
            )  # Assume activity is repeatable unless told otherwise
            self.members = 0  # for tracking # of campers assigned to this activity
            self.popularity_1 = (
                0
            )  # for tracking how many campers want the activity when calculating popularity
            self.popularity_2 = (
                0
            )  # for tracking how many campers want the activity when calculating popularity
            self.popularity_3 = (
    ...
    for i in range(sheet.ncols):
        ...
        if (
            "activ" in sheet.cell_value(0, i).lower()
            or "peula" in sheet.cell_value(0, i).lower()
        ):
            for j in range(sheet.nrows - 1):
                if sheet.cell_type(j + 1, i) != xlrd.XL_CELL_EMPTY:  # Check if empty
        ...
        # Adds capacity to each activity object
            ...
            "capacity" in sheet.cell_value(0, i).lower()
            or "max" in sheet.cell_value(0, i).lower()
        ):
            for j in range(sheet.nrows - 1):
                if sheet.cell_type(j + 1, i) != xlrd.XL_CELL_EMPTY:  # Check if empty
...
def update_campers(campers_list, excel_location):
    ...
    wb = xlrd.open_workbook(
        file_contents=excel_location.read()
    ...
    for k in range(sheet.ncols):  # k represents the excel column that contains names
        if (
            "name" in sheet.cell_value(0, k).lower()
            or "camper" in sheet.cell_value(0, k).lower()
        ):  # Find names column
            for i in range(sheet.nrows - 1):  # Loop over names in the new spreadsheet
                for j in range(len(campers_list)):  # Loop over names in camper list
                    if (
                        campers_list[j].name.lower()
                        == sheet.cell_value(i + 1, k).lower()
                    ):  # Identifies camper object corresponding to spreadsheet row
                        for l in range(
                            sheet.ncols
                        ):  # Loop over columns to gather relevant information
                            if (
                                "activity" in sheet.cell_value(0, l).lower()
                                or "peula" in sheet.cell_value(0, l).lower()
                            ):
                                if (
                                    sheet.cell_type(i + 1, l) != xlrd.XL_CELL_EMPTY
                                ):  # Check if empty
                                    campers_list[j].past_activities.append(
                                        sheet.cell_value(i + 1, l)
                                    )
                            elif (
                                "pref" in sheet.cell_value(0, l).lower()
                                or "choice" in sheet.cell_value(0, l).lower()
                            ):
                                if (
                                    sheet.cell_type(i + 1, l) != xlrd.XL_CELL_EMPTY
                                ):  # Check if empty
                                    campers_list[j].past_preferences.append(
                                        sheet.cell_value(i + 1, l)
                                    )
                                    campers_list[j].avg_pref += sheet.cell_value(
                                        i + 1, l
                                    )
    ...
    for i in range(len(campers_list)):
        if len(campers_list[i].past_activities) != 0:
            campers_list[i].avg_pref = campers_list[i].avg_pref / len(
                campers_list[i].past_activities
            )
...
def sort_campers(updated_campers_list, created_activities_list):
    ...
    for i in range(len(updated_campers_list)):
        for j in range(len(created_activities_list)):
            if (
                updated_campers_list[i].pref_1.lower()
                == created_activities_list[j].name.lower()
                or updated_campers_list[i].pref_2.lower()
                == created_activities_list[j].name.lower()
            ):
    ...
    for i in range(len(created_activities_list)):
        if (
            created_activities_list[i].popularity_1
            <= created_activities_list[i].capacity
        ):  # Activities with enough spots
            for j in range(len(updated_campers_list)):
                # Assign if unpopular activity is 1st choice
                if (
                    updated_campers_list[j].pref_1.lower()
                    == created_activities_list[i].name.lower()
                ):  # If a camper's first choice
                    if created_activities_list[
                        i
                    ].repeatability or created_activities_list[i].name.lower() not in [
                        x.lower() for x in updated_campers_list[j].past_activities
                    ]:  # As long as no illegal repeat
                        assign_activity(
                            updated_campers_list[j], created_activities_list[i], 1
                        )
                # Assign if unpopular activity is 2nd choice
                elif (
                    updated_campers_list[j].pref_2.lower()
                    == created_activities_list[i].name.lower()
                ):  # If a camper's 2nd choice
                    if created_activities_list[
                        i
                    ].repeatability or created_activities_list[i].name.lower() not in [
                        x.lower() for x in updated_campers_list[j].past_activities
                        ...
                        assign_activity(
                            updated_campers_list[j], created_activities_list[i], 2
                        )  # Assign the activity
    ...
    for i in range(len(updated_campers_list)):
        if (
            updated_campers_list[i].next_activity == ""
        ):  # Only consider those still unassigned
            for j in range(len(created_activities_list)):
                if (
                    updated_campers_list[i].pref_1.lower()
                    == created_activities_list[j].name.lower()
                    ...
                    created_activities_list[
                        j
                    ].popularity_1 += 1  # Update popularity of 1st choice for requests
                if (
                    updated_campers_list[i].pref_2.lower()
                    == created_activities_list[j].name.lower()
                ):
                    created_activities_list[
                        j
                    ].popularity_2 += 1  # Update popularity of 2nd choice for requests
                    created_activities_list[
                        j
                    ].popularity_3 += (
                        1
                    )  # 3rd choice popularity reflects 2nd choices handed out first
                if (
                    updated_campers_list[i].pref_3.lower()
                    == created_activities_list[j].name.lower()
                ):
                    created_activities_list[
                        j
                    ].popularity_3 += 1  # Update popularity of 3rd choice for requests
    for i in range(len(created_activities_list)):
        if (
            created_activities_list[i].capacity
            - created_activities_list[i].members
            - created_activities_list[i].popularity_1
        ) <= 0:  # If no spots left after 1st choice
            ...
            created_activities_list[i].popularity_2 = created_activities_list[
                i
            ].popularity_2 / (
                created_activities_list[i].capacity - created_activities_list[i].members
            )  # Calculate popularity value
            created_activities_list[i].popularity_3 = created_activities_list[
                i
            ].popularity_3 / (
                created_activities_list[i].capacity - created_activities_list[i].members
            )  # Calculate popularity value
    ...
    for j in range(len(created_activities_list)):
        ...
        if (
            created_activities_list[j].popularity_1
            <= created_activities_list[j].capacity
        ):  # Activities with enough spots
            for i in range(len(updated_campers_list)):
                if (
                    updated_campers_list[i].pref_1.lower()
                    == created_activities_list[j].name.lower()
                    and updated_campers_list[i].next_activity == ""
                ):
                    if created_activities_list[
                        j
                    ].repeatability or created_activities_list[j].name.lower() not in [
                        x.lower() for x in updated_campers_list[i].past_activities
                    ]:  # Check for illegal repeats
                        assign_activity(
                            updated_campers_list[i], created_activities_list[j], 1
                        )
        ...
        # If an activity doesn't have enough spots, sort by 2nd and 3rd choices of campers requesting
            ...
            created_activities_list[j].popularity_1
            > created_activities_list[j].capacity
        ):  # Activities w/contested spots
            temp_list = []  # Create temp list to sort
            counter = 0
            for i in range(len(updated_campers_list)):
                if updated_campers_list[i - counter].next_activity == "":
                    if (
                        updated_campers_list[i - counter].pref_1.lower()
                        == created_activities_list[j].name.lower()
                    ):  # Campers that want this activity
                        temp_list.append(
                            updated_campers_list.pop(i - counter)
                        )  # Place them in a new list
                        counter += 1  # Ensure index not out of range
            for k in range(len(temp_list)):  # Campers that want this activity
                for m in range(
                    len(created_activities_list)
                ):  # Gather popularity values
                    if (
                        temp_list[k].pref_2.lower()
                        == created_activities_list[m].name.lower()
                    ):
                        temp_list[k].pop_2 = created_activities_list[m].popularity_2
                    if (
                        temp_list[k].pref_3.lower()
                        == created_activities_list[m].name.lower()
                    ):
            ...
            while index >= 0:
                for i in range(index):
                    if (
                        temp_list[i].pop_2 < temp_list[i + 1].pop_2
                    ):  # If easier to fill first camper's 2nd pref, switch
                        temp_list[i], temp_list[i + 1] = temp_list[i + 1], temp_list[i]
                    elif (
                        temp_list[i].pop_2 == temp_list[i + 1].pop_2
                    ):  # If equal difficulty...
                        if (
                            temp_list[i].pop_3 < temp_list[i + 1].pop_3
                        ):  # If easier to fill first camper's 3rd pref, switch
                            ...
                            if (
                                temp_list[i].had_first
                                and not temp_list[i + 1].had_first
                            ):  # If 1st camper had first, not 2nd,  switch
                                temp_list[i], temp_list[i + 1] = (
                                    temp_list[i + 1],
                                    temp_list[i],
                                )
            ...
            for i in range(len(temp_list)):
                if (
                    created_activities_list[j].members
                    < created_activities_list[j].capacity
                ):
                    if created_activities_list[
                        j
                    ].repeatability or created_activities_list[j].name.lower() not in [
                        x.lower() for x in temp_list[i - counter].past_activities
                    ]:  # As long as no illegal repeat
                        assign_activity(
                            temp_list[i - counter], created_activities_list[j], 1
                        )
    ...
    for j in range(len(created_activities_list)):
        if created_activities_list[j].popularity_1 <= (
            created_activities_list[j].capacity - created_activities_list[j].members
        ):  # Activities with enough spots for all 2nd choice requests
            for i in range(len(updated_campers_list)):
                # Assign if camper is currently unassigned and activity is 2nd choice
                if (
                    updated_campers_list[i].next_activity == ""
                ):  # Only unassigned campers
                    if (
                        updated_campers_list[i].pref_2.lower()
                        == created_activities_list[j].name.lower()
                    ):  # If a camper's 2nd choice
                        if created_activities_list[
                            j
                        ].repeatability or created_activities_list[
                            j
                        ].name.lower() not in [
                            x.lower() for x in updated_campers_list[i].past_activities
                        ]:  # No illegal repeat
                            assign_activity(
                                updated_campers_list[i], created_activities_list[j], 2
                            )
        ...
        if (
            ...
            for j in range(len(created_activities_list)):
                if (
                    ...
                    created_activities_list[
                        j
                    ].popularity_2 += 1  # Update popularity of 3rd choice for requests
                    created_activities_list[
                        j
                    ].popularity_3 += (
                        1
                    )  # 4th choice popularity reflects 3rd choices handed out first
                if (
                    updated_campers_list[i].pref_4.lower()
                    == created_activities_list[j].name.lower()
                ):
                    created_activities_list[
                        j
                    ].popularity_3 += 1  # Update popularity of 4th choice for requests
    for i in range(len(created_activities_list)):
        if (
            created_activities_list[i].capacity
            - created_activities_list[i].members
            - created_activities_list[i].popularity_1
        ) <= 0:  # If no spots left after 2nd choice
            created_activities_list[i].popularity_2 = 10000  # Set popularity super high
            created_activities_list[i].popularity_3 = 10000  # Set popularity super high
        else:  # If spots left
            created_activities_list[i].popularity_2 = created_activities_list[
    ...
    for j in range(len(created_activities_list)):
        if created_activities_list[j].popularity_1 > (
            ...
            for i in range(len(updated_campers_list)):
                if updated_campers_list[i - counter].next_activity == "":
                    if (
                        updated_campers_list[i - counter].pref_2.lower()
                        == created_activities_list[j].name.lower()
                    ):  # Campers that want this activity
                        temp_list.append(
                            updated_campers_list.pop(i - counter)
            ...
            while index >= 0:
                for i in range(index):
                    if (
                        ...
                        if (
                            temp_list[i].pop_3 < temp_list[i + 1].pop_3
                        ):  # If easier to fill first camper's 4th pref, switch
                            temp_list[i], temp_list[i + 1] = (
                                temp_list[i + 1],
                                temp_list[i],
                            )
                            ...
                            if (
                                temp_list[i].avg_pref < temp_list[i + 1].avg_pref
                            ):  # If one camper previously got higher choices
                                temp_list[i], temp_list[i + 1] = (
                                    temp_list[i + 1],
                                    temp_list[i],
                                )
            ...
            for i in range(len(temp_list)):
                if (
                    ...
                    if created_activities_list[
                        ...
                        assign_activity(
                            temp_list[i - counter], created_activities_list[j], 2
                        )
                updated_campers_list.append(temp_list.pop(i - counter))
                counter += 1
    ...
    for j in range(len(created_activities_list)):
        if created_activities_list[j].popularity_1 <= (
            created_activities_list[j].capacity - created_activities_list[j].members
        ):  # Activities with enough spots for all 3rd choice requests
            for i in range(len(updated_campers_list)):
                ...
                if (
                    ...
                    if (
                        ...
                        if created_activities_list[
                            ...
                            assign_activity(
                                updated_campers_list[i], created_activities_list[j], 3
                            )
        ...
        if (
            ...
            for j in range(len(created_activities_list)):
                if (
                    ...
                    created_activities_list[
                        j
                    ].popularity_2 += 1  # Update popularity of 4th choice for requests
                    created_activities_list[
                        j
                    ].popularity_3 += (
                        1
                    )  # 5th choice popularity reflects 4th choices handed out first
                if (
                    updated_campers_list[i].pref_5.lower()
                    == created_activities_list[j].name.lower()
                ):
                    created_activities_list[
                        j
                    ].popularity_3 += 1  # Update popularity of 5th choice for requests
    for i in range(len(created_activities_list)):
        if (
            created_activities_list[i].capacity
            - created_activities_list[i].members
            - created_activities_list[i].popularity_1
        ) <= 0:  # If no spots left after 3rd choice
            created_activities_list[i].popularity_2 = 10000  # Set popularity super high
            created_activities_list[i].popularity_3 = 10000  # Set popularity super high
        else:  # If spots left
            created_activities_list[i].popularity_2 = created_activities_list[
    ...
    for j in range(len(created_activities_list)):
        if created_activities_list[j].popularity_1 > (
            ...
            counter = (
                0
            )  # Used to track members pulled from original list, keeps index in range
            for i in range(len(updated_campers_list)):
                if updated_campers_list[i - counter].next_activity == "":
                    if (
                        updated_campers_list[i - counter].pref_3.lower()
                        == created_activities_list[j].name.lower()
                    ):  # Campers that want this activity
                        temp_list.append(
                            updated_campers_list.pop(i - counter)
            ...
            while index >= 0:
                for i in range(index):
                    if (
                        ...
                        if (
                            temp_list[i].pop_3 < temp_list[i + 1].pop_3
                        ):  # If easier to fill first camper's 5th pref, switch
                            temp_list[i], temp_list[i + 1] = (
                                temp_list[i + 1],
                                temp_list[i],
                            )
            ...
            for i in range(len(temp_list)):
                if (
                    ...
                    if created_activities_list[
                        ...
                        assign_activity(
                            temp_list[i - counter], created_activities_list[j], 3
                        )
                updated_campers_list.append(temp_list.pop(i - counter))
                counter += 1
    ...
    for j in range(len(created_activities_list)):
        if created_activities_list[j].popularity_1 <= (
            created_activities_list[j].capacity - created_activities_list[j].members
        ):  # Activities with enough spots for all 4th choice requests
            for i in range(len(updated_campers_list)):
                ...
                if (
                    ...
                    if (
                        ...
                        if created_activities_list[
                            ...
                            assign_activity(
                                updated_campers_list[i], created_activities_list[j], 4
                            )
        ...
        if (
            ...
            for j in range(len(created_activities_list)):
                if (
                    ...
                    created_activities_list[
                        j
                    ].popularity_2 += 1  # Update popularity of 5th choice for requests
                    created_activities_list[
                        j
                    ].popularity_3 += (
                        1
                    )  # 6th choice popularity reflects 5th choices handed out first
                if (
                    updated_campers_list[i].pref_6.lower()
                    == created_activities_list[j].name.lower()
                ):
                    created_activities_list[
                        j
                    ].popularity_3 += 1  # Update popularity of 6th choice for requests
    for i in range(len(created_activities_list)):
        if (
            created_activities_list[i].capacity
            - created_activities_list[i].members
            - created_activities_list[i].popularity_1
        ) <= 0:  # If no spots left after 4th choice
            created_activities_list[i].popularity_2 = 10000  # Set popularity super high
            created_activities_list[i].popularity_3 = 10000  # Set popularity super high
        else:  # If spots left
            created_activities_list[i].popularity_2 = created_activities_list[
    ...
    for j in range(len(created_activities_list)):
        if created_activities_list[j].popularity_1 > (
            ...
            for i in range(len(updated_campers_list)):
                if updated_campers_list[i - counter].next_activity == "":
                    if (
                        updated_campers_list[i - counter].pref_4.lower()
                        == created_activities_list[j].name.lower()
                    ):  # Campers that want this activity
                        temp_list.append(
                            updated_campers_list.pop(i - counter)
            ...
            while index >= 0:
                for i in range(index):
                    if (
                        ...
                        if (
                            temp_list[i].pop_3 < temp_list[i + 1].pop_3
                        ):  # If easier to fill first camper's 6th pref, switch
                            temp_list[i], temp_list[i + 1] = (
                                temp_list[i + 1],
                                temp_list[i],
                            )
            ...
            for i in range(len(temp_list)):
                if (
                    ...
                    if created_activities_list[
                        ...
                        assign_activity(
                            temp_list[i - counter], created_activities_list[j], 4
                        )
                updated_campers_list.append(temp_list.pop(i - counter))
                counter += 1
    ...
    for j in range(len(created_activities_list)):
        if created_activities_list[j].popularity_1 <= (
            created_activities_list[j].capacity - created_activities_list[j].members
        ):  # Activities with enough spots for all 5th choice requests
            for i in range(len(updated_campers_list)):
                ...
                if (
                    ...
                    if (
                        ...
                        if created_activities_list[
                            ...
                            assign_activity(
                                updated_campers_list[i], created_activities_list[j], 5
                            )
        ...
        if (
            ...
            for j in range(len(created_activities_list)):
                if (
                    ...
                    created_activities_list[
                        j
                    ].popularity_2 += 1  # Update popularity of 6th choice for requests
    for i in range(len(created_activities_list)):
        if (
            created_activities_list[i].capacity
            - created_activities_list[i].members
            - created_activities_list[i].popularity_1
        ) <= 0:  # If no spots left after 5th choice
            created_activities_list[i].popularity_2 = 10000  # Set popularity super high
        else:  # If spots left
            created_activities_list[i].popularity_2 = created_activities_list[
                i
    ...
    for j in range(len(created_activities_list)):
        if created_activities_list[j].popularity_1 > (
            ...
            for i in range(len(updated_campers_list)):
                if updated_campers_list[i - counter].next_activity == "":
                    if (
                        updated_campers_list[i - counter].pref_5.lower()
                        == created_activities_list[j].name.lower()
                    ):  # Campers that want this activity
                        temp_list.append(
                            updated_campers_list.pop(i - counter)
            ...
            for i in range(len(temp_list)):
                if (
                    ...
                    if created_activities_list[
                        ...
                        assign_activity(
                            temp_list[i - counter], created_activities_list[j], 5
                        )
                updated_campers_list.append(temp_list.pop(i - counter))
                counter += 1
    ...
    for j in range(len(created_activities_list)):
        if created_activities_list[j].popularity_1 <= (
            created_activities_list[j].capacity - created_activities_list[j].members
        ):  # Activities with enough spots for all 6th choice requests
            for i in range(len(updated_campers_list)):
                ...
                if (
                    ...
                    if (
                        ...
                        if created_activities_list[
                            ...
                            assign_activity(
                                updated_campers_list[i], created_activities_list[j], 6
                            )
    ...
    for j in range(len(created_activities_list)):
        if created_activities_list[j].popularity_1 > (
            ...
            for i in range(len(updated_campers_list)):
                if updated_campers_list[i - counter].next_activity == "":
                    if (
                        updated_campers_list[i - counter].pref_6.lower()
                        == created_activities_list[j].name.lower()
                    ):  # Campers that want this activity
                        temp_list.append(
                            updated_campers_list.pop(i - counter)
            ...
            for i in range(len(temp_list)):
                if (
                    ...
                    if created_activities_list[
                        ...
                        assign_activity(
                            temp_list[i - counter], created_activities_list[j], 6
                        )
                updated_campers_list.append(temp_list.pop(i - counter))
                counter += 1

    if are_campers_sorted(
        updated_campers_list
    ):  # Need if statement because clean function assumes full camper list
        clean(updated_campers_list, created_activities_list)
    else:
        counter = 0
        temporary_list = []
...
def clean(campers_list, activities_list):
    ...
    while tracker != 0:
        ...
        for i in range(len(campers_list)):
            if (
                campers_list[i].past_preferences[
                    len(campers_list[i].past_preferences) - 1
                ]
                == 6
            ):  # If you got your 6th choice
                for j in range(len(activities_list)):
                    if (
                        activities_list[j].name == campers_list[i].pref_5
                    ):  # Check 5th choice for openings
                ...
                for j in range(len(activities_list)):
                    if (
                        activities_list[j].name == campers_list[i].pref_4
                    ):  # Check 4th choice for openings
                        if activities_list[j].capacity > activities_list[j].members:
                            remove_activity(campers_list[i], activities_list)
                ...
                for j in range(len(activities_list)):
                    if (
                        activities_list[j].name == campers_list[i].pref_3
                    ):  # Check 3rd choice for openings
                        if activities_list[j].capacity > activities_list[j].members:
                            remove_activity(campers_list[i], activities_list)
                ...
                for j in range(len(activities_list)):
                    if (
                        activities_list[j].name == campers_list[i].pref_2
                    ):  # Check 2nd choice for openings
                        if activities_list[j].capacity > activities_list[j].members:
                            remove_activity(campers_list[i], activities_list)
                ...
                for j in range(len(activities_list)):
                    if (
                        activities_list[j].name == campers_list[i].pref_1
                    ):  # Check 1st choice for openings
                        if activities_list[j].capacity > activities_list[j].members:
                            remove_activity(campers_list[i], activities_list)
    ...
    for i in range(len(campers_list)):
        if (
            campers_list[i].past_preferences[len(campers_list[i].past_preferences) - 1]
            == 1
        ):  # Find campers w/ 1st choice
            for j in range(len(campers_list)):
                if (
                    campers_list[j].past_preferences[
                        len(campers_list[j].past_preferences) - 1
                    ]
                    == 3
                    or campers_list[j].past_preferences[
                        len(campers_list[j].past_preferences) - 1
                    ]
                    == 4
                    or campers_list[j].past_preferences[
                    ...
                    == 6
                ):  # Compare all campers with 3+ choice
                    if (
                        campers_list[i].pref_2 == campers_list[j].next_activity
                        and campers_list[j].pref_2 == campers_list[i].next_activity
                    ):  # If activities are others' 2nd prefs
                        campers_list[i].next_activity, campers_list[j].next_activity = (
                            campers_list[j].next_activity,
                            campers_list[i].next_activity,
                        )  # Switch activities
                        campers_list[j].past_preferences[
                            len(campers_list[j].past_preferences) - 1
